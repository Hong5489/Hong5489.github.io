...
After some research, we can modify the size of PNG using Hexeditor, based on [this article](https://www.linkedin.com/pulse/hex-editing-width-height-png-files-ciaran-mc-ardle/)
...
Its try to edit the size using hexeditor
...
Notice the `MessageFromAuthor.txt` we saw before, we can guess the code below is just importing `CreateFileA` `WriteFile` from `kernel32.dll` and create the file with the name and write the content

After that, you will see a large chunk of code in base64:
```cs
...
Basically the base64 is assembly machine code, you can see it decode it and pass it into `Assembly.Load`:
```cs
Type type = Assembly.Load(new bsd(Encoding.UTF8.GetString(bytes).ToCharArray()).gdgdgd()).GetType("Baby_Sharp.shark_call");
```
...
Lets try what will happen when we delete the file in middle
...
# Summary
I was a great CTF, can see it getting harder compare to last year. The challenges all are great and nice, this is the last year we participate for this competition because we graduated and this competition is only for Uni students. Hope in future, our juniors can participate and win the prize!